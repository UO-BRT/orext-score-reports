...
```{r }
stu_data <- data.frame(
  var = c("SSID", "Grade", "Birthdate", "Best Score", "Test Date",
          "Teacher", "School", "District", "County"),
  id = c(rep(NA_integer_, 6), 
         params$school_id, params$district_id, params$county_id),
  values = with(params, 
                c(ssid, grade, birthdate, bestscore, testdate, 
...
```{r plot, fig.height = 1, out.width='743px', dpi = 600}
...
minmaxes <- data.frame(
  grade = c(3:8, 11, 12),
  min = c(140, 145, 145, 155, 155, 160, 850, 850),
  max = c(245, 260, 260, 260, 260, 260, 960, 960)
)

score_min <- minmaxes[minmaxes$grade == params$grade, "min"]
score_max <- minmaxes[minmaxes$grade == params$grade, "max"]
...
plot_d <- data.frame(
  content = content,
  score = ifelse(params$score < score_min, score_min, ifelse(
    params$score > score_max, score_max, params$score
    )
  )
)
...
# pad <- max(diff(c(tbl$c1, tbl$c2, tbl$c3)))
# min <- tbl$c1 - pad
# max <- tbl$c3 + pad
# 
# plot_d <- data.frame(
...
#     ),
#   ) +
#   geom_vline(aes(xintercept = c1), tbl,
#              linetype = "dashed") +
#   geom_vline(aes(xintercept = c2), tbl,
#            linetype = "dashed") +
#   geom_vline(aes(xintercept = c3), tbl,
#            linetype = "dashed") +
#   geom_point(
#     shape = 23, 
#     fill = "#B9CDE5", 
...
#   ) +
#   scale_x_continuous(
#     limits = range(c(tbl$c1 - pad, tbl$c3 + pad)),
#     name = "",
#     breaks = NULL
#   ) +
#   scale_y_continuous(
...
amo <- assign_amo(c1 = tbl$c1, c2 = tbl$c2, c3 = tbl$c3, params$score)
...
color_tbl <- function(amo) {
  ...
  body <- paste0(
    "background-color: #57cc99; color: #FFFFFF;}"
  )
  paste0(topline, body)
}
```
